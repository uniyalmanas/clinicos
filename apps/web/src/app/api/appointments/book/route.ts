...
export async function POST(req: NextRequest) {
  try {
    ...
    // Compute next sequential token number for this doctor on this date
    ...
    // If the patient had a completed consultation within validity days (default 7), consultation is free follow-up
    const validityDays = doc?.followup_validity_days || 7;
    const pastConsultations = await sql`
    ...
    const isFreeFollowup = pastConsultations.length > 0;
