          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500">Zero-Commission Generic Medicine Fulfillment Hub</p>
              </div>
            </div>
          </div>
      ...
      <main className="flex-1 mx-auto max-w-7xl w-full px-4 py-8 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Incoming Orders Today</span>
            <div className="mt-2 text-3xl font-black text-slate-900 dark:text-white">{orders.length}</div>
            <p className="mt-1 text-xs text-slate-500">From Derma Care & Smile Craft clinics</p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">NMC Generic Compliance</span>
            <div className="mt-2 text-3xl font-black text-brand-600">100%</div>
            <p className="mt-1 text-xs text-slate-500">All generic molecules verified</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {orders.map(order => (
              ...
              >
                ...
                {/* ITEMS TABLE */}
                <div className="rounded-2xl bg-slate-50 p-4 border border-slate-100 dark:bg-slate-950/50 dark:border-slate-850">
                    ...
                    {order.items.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between text-xs">
                        <div>
                          ...
                          <span className="text-slate-500 ml-2 font-normal">({item.instructions})</span>
                        </div>
                        <span className="rounded bg-white px-2 py-0.5 font-bold text-slate-700 border border-slate-200 dark:bg-slate-900 dark:border-slate-800 dark:text-slate-200">
                          {item.qty}
                        </span>
                      </div>
                    ))}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Link
                      ...
                      target="_blank"
                      className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-50 dark:border-slate-800 dark:text-slate-300"
